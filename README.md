...
Figure 1, `model/plot_trajectory.py` for Figure 2, and `model/gen_geometry.py`,
which emits
...
docs/figures/   generated figures (model/plot_dh.py, model/plot_trajectory.py)
```
...
<picture>
  ...
  <img alt="Left: the pentagon trajectory in the base x-y plane, drawn against the manipulator's wrist-centre reachable envelope for scale, with its five vertices labelled by commanded tool yaw and the sample density along each edge shown as a row of dots. Right: a schematic of the linear yaw ramp along one edge, arrows showing the tool-frame orientation advancing smoothly from 0 to 90 degrees between two vertices rather than stepping at either end." src="docs/figures/trajectory.svg" width="100%">
</picture>

**Figure 2.** Workload B's geometry. Both panels are generated by
`model/plot_trajectory.py` directly from the `TRAJ_*` constants in
`model/gen_vectors.py`, so the figure cannot state a centre, radius or sample
count that disagrees with the table actually emitted. The pentagon occupies a
small fraction of the reachable envelope — deliberately: §4.2 explains that its
siting is chosen for worst-case elbow conditioning along the path, not for
reach. Panel B is the correction described in the note above: an earlier
version of this workload stepped tool yaw discontinuously at each vertex,
which presented the DLS solver with a step input on every control period: the
ramp shown removes it.
...
python3 model/plot_trajectory.py   # optional: regenerate Figure 2
...
`docs/figures/*.svg` is likewise generated — by `model/plot_dh.py` (Figure 1)
and `model/plot_trajectory.py` (Figure 2) — and checked in, so the README
renders on a fresh clone. Neither has a build-time consumer or a staleness
guard, so a DH change or a `TRAJ_*` change requires re-running the relevant
script by hand.
