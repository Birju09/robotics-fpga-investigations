...
int main()
{
    ...
    for (int v = 0; v < nvec; v++) {
        ...
        for (int i = 0; i < IK_DOF; i++) { long w; f >> w; pose[i] = (ik_word_t)w; }
        for (int i = 0; i < IK_DOF; i++) { long w; f >> w; seed[i] = (ik_word_t)w; }
        for (int i = 0; i < IK_DOF; i++) { long w; f >> w; gold[i] = tb_dbl((ik_word_t)w); }
        f >> gold_iters;
        (void)gold;

        ik_word_t q[IK_DOF], resid;
        int iters = -1, status = -1;
        ik_dls_kernel(pose, seed, LAMBDA, TOLW, IK_DLS_MAX_ITER,
                      q, &iters, &resid, &status);

        total++;
        if (status == IK_OK) converged++;
        else {
        ...
        if (iters >= 0) {
            if ((int)hist.size() <= iters) hist.resize(iters + 1, 0);
            hist[iters]++;
        }

        ik_real_t qr[IK_DOF];
        for (int i = 0; i < IK_DOF; i++) qr[i] = ik_from_word(q[i]);
        ik_real_t R[3][3], p[3];
        ikk::fk(qr, R, p);
    ...
    if (cnt) {
        std::printf("  iterations: min=%d median=%d p95=%d max=%d mean=%.2f\n",
                    mn, all[all.size() / 2],
                    all[(size_t)(all.size() * 0.95)], mx, (double)sum / cnt);
