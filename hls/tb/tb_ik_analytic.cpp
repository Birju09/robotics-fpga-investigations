...
int main()
{
    ...
    for (int v = 0; v < nvec; v++) {
        ...
        for (int i = 0; i < IK_DOF; i++)
            worst = std::max(worst, std::fabs(tb_angdiff(tb_dbl(q[i]), gold[i])));

        if (cond >= COND_MIN) {
            for (int i = 0; i < IK_DOF; i++)
                jt.note(std::fabs(tb_angdiff(tb_dbl(q[i]), gold[i])), TOL_JOINT);
        } else {
            n_ill++;
        ...
        for (int i = 0; i < 3; i++) {
            ...
            for (int j = 0; j < 3; j++)
                dr = std::max(dr, std::fabs((double)R[i][j] - (double)Rd[i][j]));
        }
    ...
    std::printf("  %-28s %d vectors, worst joint delta %.3e (reported, not asserted)\n",
