...
//
//! The ROBOT lives in ik_geometry.hpp, which is generated by
//! model/gen_geometry.py from model/robot.py.  It used to live here, as a
//! second copy of the constants in model/ik_model.py kept in agreement by
//! hand; that is a bad arrangement for a project whose output is a comparison
//! between the Python reference and this kernel, because a silent divergence
//! does not look like a build error, it looks like quantisation error.
//
//! Everything below is a genuine implementation choice rather than a property
//! of the arm, which is why it is still written by hand.
//
