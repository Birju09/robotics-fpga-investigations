...
#pragma HLS UNROLL
    ...
    /* ---- 2. base rotation, and the in-plane radius, from one CORDIC ---- */
    ik_real_t t1b, rad;
    ikm::atan2_hypot(pc[1], pc[0], t1b, rad);

    ik_real_t t1 = sh ? t1b
                      : ikm::wrap_pi((ik_real_t)((ik_acc_t)t1b + (ik_acc_t)IK_PI));

    /* Reaching backwards flips the sign of the planar radius. */
    ik_acc_t rad_s = sh ? (ik_acc_t)rad : (ik_acc_t)(-rad);
    ik_real_t r = (ik_real_t)(rad_s - (ik_acc_t)IK_A1);
    ik_real_t s = (ik_real_t)((ik_acc_t)pc[2] - (ik_acc_t)IK_D1);

    /* ---- 3. planar 2-link sub-problem ----
     * r = a2 cos(t2) + L3 cos(beta),  s = a2 sin(t2) + L3 sin(beta),
     * with beta = t2 + t3 - PHI and gamma = beta - t2. */
    ik_acc_t rs = (ik_acc_t)(r * r) + (ik_acc_t)(s * s)
                  - (ik_acc_t)IK_A2SQ_PLUS_L3SQ;
    ...
    ik_real_t sg_mag = ikm::sqrt((ik_real_t)((ik_acc_t)1 - (ik_acc_t)(cg * cg)));
    ik_real_t sg = el ? sg_mag : (ik_real_t)(-sg_mag);

    ik_real_t gamma = ikm::atan2(sg, cg);
    ik_real_t t3 = ikm::wrap_pi((ik_real_t)((ik_acc_t)gamma + (ik_acc_t)IK_PHI));

    ik_real_t num = (ik_real_t)((ik_real_t)IK_L3 * sg);
    ik_real_t den = (ik_real_t)((ik_acc_t)IK_A2
                                + (ik_acc_t)((ik_real_t)IK_L3 * cg));
...
AN_R36_R:
    ...
    if (s5 < (ik_real_t)IK_WRIST_EPS) {
        ...
        t4 = ikm::atan2(R36[1][2], R36[0][2]);
        t5 = ikm::atan2(s5, R36[2][2]);
        t6 = ikm::atan2(R36[2][1], (ik_real_t)(-R36[2][0]));
    } else {
        t4 = ikm::wrap_pi((ik_real_t)((ik_acc_t)ikm::atan2(R36[1][2], R36[0][2])
                                      + (ik_acc_t)IK_PI));
        t5 = ikm::atan2((ik_real_t)(-s5), R36[2][2]);
        t6 = ikm::wrap_pi(ikm::atan2((ik_real_t)(-R36[2][1]), R36[2][0]));
    }

    q[0] = t1; q[1] = t2; q[2] = t3;
    q[3] = t4; q[4] = t5; q[5] = t6;
    return st;
}
