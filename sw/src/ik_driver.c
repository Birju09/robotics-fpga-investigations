...
#error \
    "Neither xtime_l.h nor xiltimer.h is in the BSP; no time base for the measurements."
#endif
...
static void ik_write_block(ik_dev_t* dev, uintptr_t base, const float* src,
                           int n) {
    for (int i = 0; i < n; i++)
...
int ik_analytic_solve(ik_dev_t* dev, const float pose[6], int cfg,
    ...
    if (to) {
        if (cycles)
            *cycles = (uint32_t)(ik_timer_read() - t0);
        return -1;
    }
...
int ik_dls_solve(ik_dev_t* dev, const float pose[6], const float q_seed[6],
    ...
    WR(dev, XIK_DLS_KERNEL_CTRL_ADDR_TASK_DIM_DATA, task_dim);
    ...
    if (resid)
        *resid =
            ik_q2f((ik_word_t)RD(dev, XIK_DLS_KERNEL_CTRL_ADDR_RESID_DATA));
